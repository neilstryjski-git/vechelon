...
| S7 | **`productdelivered.ca` has no SPF, DKIM or DMARC** while its MX is Google Workspace and Neil sends from `neil@productdelivered.ca`. Pre-existing gap, not caused by Vercel. | The brief says "recreate exactly". **Decision (Neil, 2026-09-22): add Google SPF, DKIM and DMARC at Cloudflare** as a separately logged step in Phase 2, after the verbatim copy is verified. DKIM key generated 2026-09-22; the three records are staged in `dns-additions-productdelivered.ca.json`. | port (addition) |
...
| Google Maps Platform key (`VITE_GOOGLE_MAPS_API_KEY`, Static Maps API in `admin/src/lib/maps.ts`) | Key `AIzaSyAL…YIzM` (identified from the deployed bundle `index-QsH1jG0y.js`). Website referrers, read by Neil from the Google Cloud console 2026-09-22: `*.vechelon.ca`, `https://vechelon-rail3-staging.vercel.app/*`, `localhost:*`, `vechelon.productdelivered.ca/*` | Only the staging entry (S2) | Phase 3: **add** the Cloudflare preview hostname(s) and the new staging host; **remove** `vechelon-rail3-staging.vercel.app` in Phase 5. Prod entries unchanged. |
| Google Play listing | privacy-policy URL = `https://vechelon.productdelivered.ca/privacy` (mobile `src/lib/env.ts` on `rail3-integration`; `privacy.html` is in `dist_production/`) | No | None, but the path must keep serving 200 on Cloudflare — today that 200 depends on the `/privacy` → `/privacy.html` rewrite (§2.1), which is in the Port column (§4), not only on `privacy.html` existing. The 2026-08-30 note "hold vechelon.productdelivered.ca on Vercel until Play verification settles" — **cleared by Neil 2026-09-22 (Play work is done for now)**; Phase 4 cuts all hostnames over together, no carve-out. |
...
| Per-club subdomains (4 Vercel domain entries + 4 Porkbun CNAMEs) | Wildcard `*.vechelon.ca` CNAME + Worker route (decided, S5), plus an unknown-slug page in the app. |
...
4. ~~Google Maps key referrer list~~ — captured 2026-09-22 (§3).
