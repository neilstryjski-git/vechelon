...
| Name | Type | Value | TTL | Points at | Note |
|---|---|---|---|---|---|
| @ | A | 66.33.60.129, 76.76.21.22 | ~600 | Vercel (vechelon) | Vercel answers 308 → vechelon.productdelivered.ca (project domain redirect). |
...
| Rewrites (`vercel.json`) | `/portal/:path*` → `/portal/index.html` (SPA) · `/` on host `vechelon.productdelivered.ca` → `/landing.html` · `/(.*)` on host `(?<slug>[a-z0-9-]+).vechelon.ca` → `/portal/index.html` · `/prototype` → `/prototype.html` · `/roadmap` → `/roadmap.html` |
...
| `vercel.json` path rules: `/portal/*` SPA fallback, `/admin/*` → `/portal/*` 301, `/ride/*` → `/portal/ride/*` 307 (host-scoped), `/prototype`, `/roadmap` | `_redirects` for path-only rules; Worker for the host-scoped `/ride` rule. |
