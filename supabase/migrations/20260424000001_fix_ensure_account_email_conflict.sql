-- Fix ensure_account_exists to handle email unique constraint conflicts gracefully.
-- ON CONFLICT (id) doesn't cover the email unique index, causing 409 when a stale
-- session tries to re-insert with an email already owned by another account.
-- Use EXCEPTION WHEN unique_violation to silently skip and continue in that case.
...
BEGIN
  ...
  EXCEPTION WHEN unique_violation THEN
    NULL; -- email taken by another account — skip and continue
  END;
