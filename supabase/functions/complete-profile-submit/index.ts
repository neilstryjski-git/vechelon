...
serve(async (req) => {
  ...
  try {
    ...
    // status, and never downgrade ROLE — completing a profile must not demote an
    // existing admin to member (an upsert with role:'member' would clobber it).
    ...
    // Land the magic link on /auth (the token-exchange route) — a bare-origin
    // redirect_to doesn't match the uri_allow_list and falls back to site_url (a
    // different host → 404). Mirrors send-magic-link.
    const portalAuthBase = `${portalUrl}/auth`
    const { data: mlData, error: mlError } = await adminClient.auth.admin.generateLink({
      type: 'magiclink',
      email: normalizedEmail,
    ...
    if (!mlError && mlData?.properties?.action_link) {
      // Wrap the raw verify URL in the /auth ?c= click-through so email scanners
      // (Gmail/Outlook) can't pre-consume the one-time token (mirrors send-magic-link).
      const signInLink = `${portalAuthBase}?c=${encodeURIComponent(btoa(mlData.properties.action_link))}`
