// Role-gated visibility rules for the live fleet map (W172 mobile → W190 web).
...
// rules are unit-testable without rendering (roleVisibility.test.ts).
//
...
//
// Ported verbatim from mobile/src/lib/roleVisibility.ts (origin/W172): pure
// TypeScript, no React Native imports.
...
// §4.1 CONTACT: phone visibility and Dial follow the same gate as the detail
// panel — Captain/SAG see any visible target's number; Riders see Captain/SAG
// numbers. Kept as a separate predicate so a future divergence (e.g. hiding
// numbers but allowing the panel) only touches this function.
