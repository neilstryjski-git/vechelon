...
const ProfileCompletionPrompt: React.FC = () => {
  ...
  // Prompt on either gap: a missing NAME (roster identity) or a missing PHONE
  // (reachability by support/captain on a ride — safety-critical).
  const needsName = !account?.name?.trim();
  const needsPhone = !account?.phone?.trim();
  const incomplete = userTier !== 'guest' && !!account && (needsName || needsPhone);
  if (!incomplete || dismissed) return null;

  const nudgeCopy = needsName
    ? 'Add your name so your club knows who you are on the roster.'
    : 'Add your phone so support and your captain can reach you on a ride.';
      ...
      <div className="bg-brand-primary/10 border-b border-brand-primary/20 text-on-background z-[59] relative">
        <div className="max-w-screen-2xl mx-auto px-6 py-2.5 flex items-center justify-between gap-4">
          <div className="flex items-center gap-2.5 min-w-0">
            <span className="material-symbols-outlined text-brand-primary text-lg shrink-0">{needsName ? 'badge' : 'call'}</span>
