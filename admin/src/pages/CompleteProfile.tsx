...
const CompleteProfile: React.FC = () => {
  ...
  // Any field the club already has is shown locked ("we've got this") with an
  // inline Edit — so the rider only has to touch the gaps (usually just email).
  const [locked, setLocked] = useState({
    name: false, email: false, phone: false, ecName: false, ecPhone: false,
  });
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Lock every field the club already supplied; leave the gaps editable.
      setLocked({
        name:    !!data.name,
        email:   !!data.email,
        phone:   !!data.phone,
        ecName:  !!data.emergency_contact_name,
        ecPhone: !!data.emergency_contact_phone,
      });
      setStage('form');
    })();
  ...
  const unlock = (key: keyof typeof locked) => setLocked((l) => ({ ...l, [key]: false }));

  // A field the club already has renders locked (value + inline Edit); an empty
  // field renders as a normal editable input.
  const field = (
    key: keyof typeof locked,
    label: string,
    value: string,
    ...
    <div>
      ...
      {locked[key] ? (
        <div className="w-full bg-surface-container-low border border-outline-variant/30 rounded-xl px-5 py-4 flex items-center justify-between gap-3 shadow-sm">
          <span className="font-body text-sm text-on-background truncate">{value || '—'}</span>
          <button
            type="button"
            onClick={() => unlock(key)}
            disabled={busy}
    ...
    <div className="min-h-screen bg-surface flex flex-col items-center justify-center px-6 py-12">
      ...
      <form onSubmit={handleSubmit} className="w-full max-w-sm space-y-6 animate-in fade-in duration-700">
        ...
        <div className="space-y-4">
          {field('email', 'Email address', email, setEmail, 'email', 'you@example.com', true)}
          {field('name', 'Full name', name, setName, 'text', 'First Last')}
          {field('phone', 'Mobile number', phone, setPhone, 'tel', '+1 555 123 4567')}
          <div className="pt-2">
            <p className="font-label text-[10px] uppercase tracking-widest text-on-surface-variant/60 mb-3 px-1">
              Emergency contact (optional)
            </p>
            <div className="space-y-4">
              {field('ecName', 'Contact name', ecName, setEcName, 'text', 'Contact name')}
              {field('ecPhone', 'Contact number', ecPhone, setEcPhone, 'tel', '+1 555 987 6543')}
            </div>
          </div>
